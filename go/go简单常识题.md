...
Go 的 Context 的数据接口包含 Deadline，Done，Err，Value，
...
还要去了解一下单向channel,如只读或者只写通道常见的异常问题
...
G无限制，理论上受内存的影响，创建一个 G 的初始栈大小为2-4K，配置一般的机器也能简简单单开启数十万个 Goroutine ，而且Go语言在 G 退出的时候还会把 G 清理之后放到 P 本地或者全局的闲置列表 gFree 中以便复用。

M限制10000，Go语言运行时初始化的时候在runtime.schedinit()中通过下面代码设置了 M 的最大数。

P常规项目直接使用默认的核心数就好了，GOMAXPROCS 开太多的时候，针对计算密集型的处理性能提升反而没那么大，IO 密集(或者 syscall 较多)的 Go 程序，至少应该配置到CPU核心数目的5倍以上, 最大1024。
...
```
https://github.com/Zeb-D/go-util/blob/master/taskpool/README.md
```
...
2. Go 里面还能通过有缓冲的 channel 实现其阻塞等待一组协程结束，这个不能保证一组 goroutine 按照顺序执行，可以并发执行协程。Go 里面能通过无缓冲的 channel 实现其阻塞等待一组协程结束，这个能保证一组 goroutine 按照顺序执行，但是不能并发执行。
...
答：一个包下可以有多个 init 函数，每个文件也可以有多个 init 函数。

多个 init 函数按照它们的文件名顺序逐个初始化。应用初始化时初始化工作的顺序是，从被导入的最深层包开始进行初始化，层层递出最后到 main 包。
...
> [详见go关于unsafe.Pointer与uintptr](./go关于unsafe.Pointer与uintptr.md)
