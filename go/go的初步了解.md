...
个人实践golang到现在有5年历史了，但接触却是在16年，也算见证了golang从一个小孩到青年的成长经历。

为什么中间隔了这么多，一直未入门，其实和吃饭的语言有关系，当时不是golang开发。



其实我当时学习golang这门语言出发点很简单，我可以写两份简历，一份是java、一份是golang（当然我现在还有一份是rust）。这样的话，那我在市场找工作在初期的时候更有优势；



随着golang生态某些优势暴露出来，那么市面golang JD就越来越多，而之前的语言会减少些，这也意味着我们在35岁？之前要实时关注新兴技术；



从19年开始，云原生架构开始兴起（kubenertes 容器化编排开始），带火了golang这块生态，这也意味着我们要入局这块云原生架构相关生态。
...
> 后续的rust也是这么玩的。构建成目标OS的可执行文件。可执行文件体积相对小。
...
> golang把GC 和内存模型这块的功能直接打包成可执行文件里面，不需要像JVM单独进行管理；
...
简单应该是 Golang 最大的优势。Golang 的语言**特性简单，学习周期短**，熟悉其他编程语言的开发者基本都可以在短时间学会并写出各方面都还不错的代码。所谓各方面都还不错是说新手开发者写出来的代码和一些有经验的开发者写出来的代码差别并不会太大。
...
> 这些很适合那些web开发，需要多起几个协程IO，对资源消耗不高。
...
> 在性能上，语言一般是编译语言、再是C、然后是rust（或者JIT后的Java 字节码）
...
> 比如在大内存下，这块相比JVM ZGC来说还是很大差异，同时GC相关参数没那么细。或者对比rust 无GC来说性能这块损失比较大。



#### 4. 生态需要继续发展

golang作为一个青年，相比Java几十年的发展路线来说，它要走的路线还是比较多，有比较多的框架、组件需要类似copy 翻译下（当然不是百分百这种）。

> 当前rust也是有这种copy行为。



可能有些人会说，在前端这块支撑力度没那么好：

> 比如java之前的JSP+servelet 模式，最终还是演变成前后端分离，所以此处我们应该把它定位成后端（可执行这种）



经过了比较多的迭代，目前golang也不断加了比较多特性：

> 1、支持将文件打包到二进制文件中，类型java的classpath这种。可见关键词 `embed`
>
> 2、范型支持，功能上比不过java（好比不过没有），但实现方式上臃肿；
>
> 3、GC 实现上不断优化调整，个人感觉极大程度会参考Java ZGC的实现；
>
> 4、语法升级，部分参考了Rust部分语法，比如 赋值+switch 方式。



### 交流几个问题

Q1：除了语法特性约束外，你还觉得golang有哪些劣势（或者）？



Q2：你认为golang的天花板在哪？它适用于哪些场景？