...
这种事务处理方式与[2PC](../../distributtion/分布式系统深入理解2PC和3PC.md)（[两阶段提交协议](../../distributtion/分布式事务中的两阶段、三阶段提交协议.md)）区别在于，两阶段提交协议的第二阶段中，需要等到所有参与者的"YES"回复才会提交事务，只要有一个参与者反馈为"NO"或者超时无反馈，都需要中断和回滚事务。
